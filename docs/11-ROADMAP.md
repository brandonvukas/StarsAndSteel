...
- ◐ `TickProcessor` with the 14-step pipeline (AI first, deterministic RNG seeded from `world.RngState`, optimistic-concurrency `RowVersion` check) — orchestrator + DeterministicRandom (LCG, persistable) + ITickStep contract landed in 1E; 6 of 14 steps wired (AiTurn → Production → Movement → AirStrike → Combat → Construction); 8 steps remaining (naval, logistics, attrition, morale recovery, news, victory check, persistence-prep, RNG advance).
