...
- ☑ **Phase 4b2 — Wonders batch 2 (3 more).** Three additional wonders shipped, all auto-discovered by the existing client Wonders panel (no client edit required):
  - **GPS Constellation** — pure intel wonder. When the owner has a built GPS Constellation, `SnapshotService` reveals every province on the map for them and bypasses submarine stealth (sats see everything, no ASW required).
  - **Carrier Strike Group** — coastal-only wonder. On completion `ConstructionStep.SpawnCarrierStrikeGroup` spawns a free veteran (`Experience=1`) `AircraftCarrier` (Strength 1000) plus two veteran `CarrierAirWing` (Strength 500, parented to the carrier) at the wonder's province. Standard `UnitBuiltEvent` fires per spawned unit so the client gets normal hub events.
  - **Cyber Command HQ** — global cyber-ops boost. Owners get a 50% discount on CyberAttack money + electronics cost (`OrderService.CyberCommandHqDiscount`, `EffectiveCyberAttackMoneyCost/ElectronicsCost`) AND the per-launch `CyberOperationsCenter` requirement is waived (the HQ is itself the global facility). `OrdersController.CyberAttack` queries `Buildings WHERE Type = CyberCommandHq` once and threads the flag through `ValidateCyberAttack` + `DebitForCyberAttack`.
  - Tests: 8 new (CSG completion spawns carrier + 2 wings + 3 events; CSG coastal validation accept/reject; Cyber HQ flag waives COC; Cyber HQ flag halves resource check; Cyber HQ flag halves debit; GPS reveals non-adjacent enemy province; GPS reveals enemy submarine without ASW). 330 pass / 27 skip.
- ☑ **Phase 4c — Random world events.** New `RandomEventStep` (slotted after `ResourceProductionStep` so booms credit on the same tick) rolls a single global event per tick (15% trigger chance, deterministic via `context.Rng`). Five event kinds: `NaturalDisaster` (destroys one random non-wonder building), `ResourceBoom` (credits owner with one bonus tick of resources from a random province), `ScientificBreakthrough` (+25 progress on a random in-flight tech), `CivilUnrest` (-20 morale on a random province), `MarketCrash` (-10% money for a random rich player). Wonders are immune to disasters. New `RandomEventOccurredEvent` + 5 `NewsTemplate` entries + new `NewsCategory.Disaster` enum value. `NewsStep` dispatches per-kind headlines. Client gets a dedicated **World Events** side-tab listing the full body of every Disaster-category news item with severity-styled left border. Tests: 11 new (1 trigger-miss + 10 per-kind happy/skip paths) using a `ScriptedRng` stub for assertion-driven determinism. 341 pass / 27 skip.
- ☑ **Phase 4d — Insurgent (Wildcard) AI personality.** New `InsurgentPlanner` activates the previously-deferred `AiPersonality.Insurgent` slot. Each tick rolls a single d100 to pick ONE of four chaotic branches: 30% random adjacent attack with NO margin check (will throw 100 MechInf at 5000 MBT — that's the personality), 30% random recruit from a chaos pool (MechInfantry/MainBattleTank/AABattery/CombatDrone/MultiroleFighter), 30% random building from a chaos pool (RecruitmentCenter/MilitaryBase/AirBase/FinancialDistrict/Refinery/SteelMill/TechPark) at a random eligible owned province, 10% idle ("watch this"). Failed attack branch falls through to recruit so the AI still does *something*. All sub-selections (which unit, which province, which target) pull from `IRandomSource` for deterministic replays. `WorldFactory` rotation swaps the third Hawk slot for the new "Free Cadres" Insurgent nation. `AiTurnStep` dispatch comment updated (no longer "deferred to Phase 3"). No new entities or migration; no UI surface (client never displayed AI personality). Tests: 11 new (idle/attack/recruit/build/fall-through/skip paths + chaos-pool walk + non-Insurgent/dead skip + unowned-province ignore) with local `ScriptedRng`. 352 pass / 27 skip.
- ☑ **Phase 4e — Sanctions / embargoes.** Directional economic action that lets any player throttle another's money production without going kinetic. Reuses the existing `DiplomaticRelation` row by adding a single `IsSanctioning bool` column (migration `Phase4e_AddSanctions`); the From→To direction is the sanctioner. Two new endpoints `POST /diplomacy/sanction` + `/lift-sanction` (free, instant, asymmetric, can stack). `DiplomacyService.Sanction`/`LiftSanction` emit the new `SanctionChange` records inside `DiplomacyMutation`; the controller upserts the directional row (creating it at default Peace if absent — sanctions are independent of treaty status). `RelationLookup` builds an `_inboundSanctionCount` dict on construction; `ResourceProductionStep` multiplies each player's per-tick **money** (only) by `max(0.25, 1 - 0.25 * count)` so 1 sanctioner = 0.75x, 2 = 0.50x, 3+ = 0.25x floor. Other resources untouched. New `SanctionChanged` SignalR event broadcast out-of-tick by `DiplomacyBroadcaster`; new `DiplomacyRejectionReason.AlreadySanctioning`/`NotCurrentlySanctioning` (both 409). `DiplomacyController.GetState` extended to surface bidirectional `IsSanctioningAtoB`/`IsSanctioningBtoA` flags on each canonical pair. Client gets `sanctionPlayer`/`liftSanction` REST helpers, hub handler with `applySanctionChanged` reducer (preserves opposite-direction flag), `findSanctions` store helper, "Sanction"/"Lift Sanction" buttons + bidirectional "Sanctioning"/"Sanctioned by" badges in the diplomacy panel. Tests: 8 new pure unit tests on `DiplomacyService.Sanction`/`LiftSanction` (happy / self / dead / dup / world-ended / lift happy / lift not-active / lift self), 7 new pure unit tests on `ResourceProductionStep` sanction math (theory: 0/1/2/3/4/10 sanctioners with floor + sanction-only-affects-money), and 2 new `[DockerFact]` integration smokes against Testcontainers SQL (round-trip through `GET /diplomacy` + canonical pair flag mapping; self-sanction 400). 367 pass / 29 skip.
- ☑ **Phase 4f — Maneuver Warfare doctrine.** Activates the Tier-2 `maneuver_warfare` Doctrine tech that was placeholder-deferred in Phase 3g (with no multi-tick movement to shave a tick from, the original framing didn't apply). Reframed as an **engagement-time bonus**: when an attacker holds the tech AND at least one of their committed stacks moved into the contested province this same tick (detected via the existing `UnitMovedEvent` already emitted by `MovementStep` — no schema change, no new `TickContext` field), the attacker side gets +20% effective strength + outgoing damage in `CombatStep`. `CombatResolver` gains a 7-arg `ResolveGround` overload taking a symmetric `attackerBonusMultiplier` parameter (parallels the Phase 3f `defenderBonusMultiplier`); the existing 6-arg overload now shims through with `attackerBonusMultiplier: 1.0` so all earlier callers remain unchanged. The bonus stacks **multiplicatively** with combined-arms, defense-in-depth, and the general bonus. `TechCatalog` description for `maneuver_warfare` rewritten from the deferred placeholder to active effect text. Tests: 4 new pure unit tests on `CombatStep` (with-tech-and-moved-in increases defender losses; with-tech-but-no-move = baseline; defender holding the tech doesn't buff anyone; stacks multiplicatively with combined-arms when attacker fields ground+air+AA). No client/UI surface — the Research panel already lists the tech and now displays the new description automatically. 371 pass / 29 skip.
